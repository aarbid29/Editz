...
export default function SizeEdit() {
  ...
  const [color, setColor] = useState(""); // New state for color input
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (file && removePrompt && color) {
      setIsTransforming(true);
      await processImage();
    } else {
      alert(
        "Please upload an image, enter the object to remove, and specify a color."
      );
    }
  };

  const processImage = async () => {
    if (!file || !removePrompt || !color) return;
    ...
    try {
      const response = await fetch("/api/replace-color", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="flex flex-col items-center p-6 font-sans">
      <h1 className="text-2xl font-bold mb-4 text-purple-400">
        Remove Object and Change Color
      </h1>
      <p className="text-gray-600 mb-6">
        Upload an image, enter the object to remove/specify the new color, and
        submit.
      </p>
      ...
      >
        <div className="flex flex-col items-center gap-4">
          ...
          <button
            ...
            disabled={
              isUploading || isTransforming || !file || !removePrompt || !color
            }
          >
            {isUploading || isTransforming
              ? "Processing..."
              : "Upload and Change Color"}
          </button>
          ...
          {processedPublicId && (
            <div className="relative flex flex-col items-center border border-gray-300 rounded-md p-4 bg-gray-50 w-80">
              ...
              <CldImage
                ...
                width={320}
                height={240}
                alt={`Image with ${removePrompt} color changed to ${color}`}
      ...
      {!uploadedImage && !file && (
        <div className="mt-8 text-center text-gray-500">
          Please upload an image and enter the object and color.
        </div>
      )}
      {uploadedImage && file && (!removePrompt || !color) && (
        <div className="mt-8 text-center text-gray-500">
          Enter the object to remove and the color to change it to, then submit.
        </div>
      )}
    </div>
  );
